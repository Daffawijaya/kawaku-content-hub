...
import { getAccountReachDaily, getMediaInsights, listRecentMedia } from "./client";
import { isInstagramConfigured } from "./config";

export type DailyAggregate = { date: string; reach: number; impressions: number; engagement: number };
...
export async function refreshDailyAnalytics(supabase: SupabaseClient): Promise<DailyAggregate[]> {
  ...
  const [reachRows, media] = await Promise.all([
  ...
  for (const r of reachRows) {
    byDate.set(r.date, { date: r.date, reach: r.reach, impressions: 0, engagement: 0 });
  }
  await Promise.all(
    media.map(async (m) => {
      ...
      if (!ins) return;
      const cur = byDate.get(d) ?? { date: d, reach: 0, impressions: 0, engagement: 0 };
